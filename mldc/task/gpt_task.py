...
from mldc.model.gpt_model import GptModel
#from mldc.trainer.retrieval import RetrievalTrainer
from mldc.trainer.gpt_trainer import GptTrainer
...
class GptTask(Task):
    class Config(ConfigBase):
        model: GptModel.Config = GptModel.Config()
        trainer: GptTrainer.Config = GptTrainer.Config(
    ...
    def from_config(cls, task_config, metadata=None, model_state=None):
        ...
        featurizer = create_featurizer(
            ...
            text_embedder_config=task_config.text_embedder)  # featurizer :: text embedder GPT2Embed
    ...
    def __init__(
        ...
        self.trainer: GptTrainer = trainer
    ...
    def train(self, train_config, rank=0, world_size=1):
        ...
        with ExitStack() as stack:
            ...
            if self.model_needs_meta_training:
                train_iter = self.data_handler.get_train_iter(rank=rank)
    ...
    def predict(self, test_path, meta_batch_spec_file):
        ...
        for result in self.trainer.predict(self.data_handler.text_embedder,
                                           self.data_handler.get_test_iter(meta_batch_spec_file=meta_batch_spec_file),
                                           self.model,
                                           self.metric_reporter):
